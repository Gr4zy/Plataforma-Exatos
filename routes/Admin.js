...
router.post('/aulas/api/aulas', exigirGestor, async function (req, res, next) {
  const { titulo, tema, descricao, link, status, materiais } = req.body;

  if (!titulo || !tema || !descricao || !link) {
    return res.status(400).json({ erro: 'Título, tema, descrição e URL da aula são obrigatórios.' });
  }
